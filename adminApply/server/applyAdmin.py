    ...
    if not BU or not name:
        ...
        try:
            ...
            if int(newStage) == 1:
                ...
                unPass_list = Asp.objects.filter(BU = BU, applyStatus = 204)
            if int(newStage) == 2:
                asp_list = Asp.objects.filter(BU = BU, applyStatus__in = [205,300,301,302,303]).update(applyStatus = 304)
                unPass_list = Asp.objects.filter(BU = BU, applyStatus = 304)
            if int(newStage) == 3:
                asp_list = Asp.objects.filter(BU = BU, applyStatus__in = [305,400,401,402,403]).update(applyStatus = 404)
                unPass_list = Asp.objects.filter(BU = BU, applyStatus = 404)
            fallSend = []
            for unPass in unPass_list:
                result = ssender.send_with_param(86, unPass.tel, 205784, [unPass.name, translateBU(BU)], sign=sms_sign, extend="", ext="123")
